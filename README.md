...
这种题目很容易想到动态规划。`dp[i][j][k]`表示在第1~i位，最小值为j，交换次数为k的情况下有多少组合方式。
...
n种（最多8）颜色的球，每个颜色的最多6个球。分成两等分，要求每份种球的颜色数相同，最终球满足要求的概率。
动态规划`dp[i][j][k][l]`表示使用前i种颜色，在第一份种放j个球，第一份中k种颜色，第二份中l种颜色共有多少种方案。

### [5456.Kth Ancestor of a Tree Node](https://leetcode.com/problems/kth-ancestor-of-a-tree-node)

5e4个点构成的树，进行5e4次查询，每次查询点p的第k次祖先。这道题目也可以动态规划, `dp[k][i]`表示节点i的第1^k祖先，k最多20位。查询时从高位到低位累计即可。
...
在构成MST中的哪些边是必不可少的，哪些是在部分MST中不可缺少的。如果一条边不参与构成MST，得到的最小生成树权重和之前不一致，则为必不可少的。如果作为必选，构成的MST得到的权重和之前一样，则为后者。
...
移动最多k次相邻数字，使得数字串最小。n有3e4规模，k有1e9，思路是每次找能移动的最小的数字，可以用树状树状减少查询复杂度。

### [1642. Furthest Building You Can Reach](https://leetcode.com/problems/furthest-building-you-can-reach/)

从左到右，在高低不同的建筑上下跳或上跳，下跳不消耗东西，上跳消耗一个梯子或高度差的砖块。求从左到右最多能到哪？
思路：二分，但是如果在二分判断里面直接验证是否可行，复杂度为`O(n*(log n)^2)`。可以把这部分提前拿出来，用小根堆先算出来打表，复杂度就降为`O(n*log_n)`了。

### [5462. String Compression II](https://leetcode.com/contest/weekly-contest-199/problems/string-compression-ii/)

删掉k个字符串，使得字符串压缩的长度变小，求最小长度。动态规划dp[i][j]表示压缩后的第i块，删掉j个字符串的结果。注意处理删除后重新连续的情况。
